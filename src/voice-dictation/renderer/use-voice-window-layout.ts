...
/** 宿主注入的窗口高度调整能力（主进程 IPC）；缺省时静默跳过。 */
export interface VoiceWindowLayoutOptions {
  resizeWindow?: (input: { height: number }) => Promise<void> | void
}
...
export function useVoiceWindowLayout(input: VoiceWindowLayoutInput, options: VoiceWindowLayoutOptions = {}): VoiceWindowLayoutResult {
  const { commitResultMessage, message, status, transcript } = input
  const { resizeWindow } = options
  ...
  const resizeVoiceWindow = React.useCallback(() => {
    ...
    void Promise.resolve(resizeWindow?.({ height: nextHeight })).catch(console.error)
  }, [resizeWindow])
