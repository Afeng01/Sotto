/** 轻量、无焦点的听写状态条，用于向第三方应用听写时保持用户当前焦点。 */

import * as React from 'react'
import { Loader2, Mic } from 'lucide-react'
import type { VoiceDictationIndicatorEvent } from '../types'

export interface VoiceDictationIndicatorProps {
  /** 订阅指示器状态事件；由宿主应用注入自己的 bridge 实现。 */
  onIndicatorState: (callback: (event: VoiceDictationIndicatorEvent) => void) => () => void
}

export function VoiceDictationIndicatorApp({ onIndicatorState }: VoiceDictationIndicatorProps): React.ReactElement {
  const [state, setState] = React.useState<VoiceDictationIndicatorEvent['state']>('preparing')
  const [volume, setVolume] = React.useState(0)
  const [transcript, setTranscript] = React.useState('')
  const [typedTranscript, setTypedTranscript] = React.useState('')

  React.useEffect(() => {
    return onIndicatorState((event) => {
